    ...
    if( empty($_POST['username']) OR empty($_POST['email']) OR empty($_POST['password']) OR empty($_POST['re-password'])){
      echo "<script>alert('Some inputs are empty')</script>";
        ...
        if($password == $repassword){
          ...
          if(strlen($email) > 22 OR strlen($username) > 20){
              echo "<script>alert('Email or user name is to big')</script>";

          }else{

            //checking for username avaialibilty
          ...
          <div class="col-md-12 mb-5">
            <form action="" method="POST" class="p-4 border rounded">
              ...
              <div class="form-group">
                ...
                <select class="selectpicker border rounded" id="job-type" name="type" data-style="btn-black" data-width="100%" data-live-search="true" title="Select User Type">
